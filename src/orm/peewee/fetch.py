...
def create_fetch_feeditems_statement(columns: typing.List[str],
    ...
    if where_cond:
        ...
        for column, values in where_cond.items():
            if isinstance(values, str) or isinstance(values, int):
                expressions.append(getattr(src.orm.peewee.models.FeedItem, column) == values)
            elif isinstance(values, list):
                expressions.append(getattr(src.orm.peewee.models.FeedItem, column).in_(values))
    else:
        expressions = []
    query = query.where(True, *expressions)
    return query
